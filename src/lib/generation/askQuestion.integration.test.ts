...
describe.skipIf(!hasRealEnv)('askQuestion', () => {
  ...
  it(
    ...
    async () => {
      ...
      const [catEmbedding, airplaneEmbedding] = await Promise.all([embed(catText), embed(airplaneText)]);
      ...
      const { error: chunksError } = await user.from('source_chunks').insert([
        ...
        { source_id: source!.id, chunk_index: 1, content: airplaneText, embedding: airplaneEmbedding },
      ]);
