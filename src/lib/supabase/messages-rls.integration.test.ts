...
describe.skipIf(!hasRealSupabaseEnv)('messages/message_citations RLS isolation', () => {
  ...
  it(
    "prevents one anonymous user from reading or modifying another user's messages and citations",
    async () => {
      ...
      const { data: sourceA, error: sourceError } = await userA
        .from('sources')
        .insert({ notebook_id: notebookA!.id, type: 'pasted_text', title: 'User A source', status: 'ready' })
        .select()
        .single();
